...
`backend.properties()` and `backend.status()` are network calls against
the Runtime service. Scoring many circuits against many backends in a
short window (e.g. a batch submission loop) would otherwise refetch the
same calibration data repeatedly. This cache is optional — nothing else
