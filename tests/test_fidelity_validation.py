...
Skipped automatically if qiskit-aer isn't installed (it's an optional
'validate' extra, not a core dependency). See scripts/validate_fidelity_estimate.py
for the full scope note on what this does and doesn't prove — in short: this
checks the analytic estimate against a simulation built from the *same*
calibration data it reads, so it's a self-consistency check on the math,
not evidence the underlying noise model matches real hardware.
"""
...
from qiskit_loadbalancer.scoring import NoiseAwareScoring  # noqa: E402

SHOTS = 8000
TOLERANCE = 0.05  # analytic vs simulated agreement observed empirically at ~0.02
